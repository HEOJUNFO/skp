<template>
	<container ref="containerEl">
		<event-scan-object v-if="imageScanningInfoList" ref="eventIsScanEl" :scanning-list="imageScanningInfoList"
			@load:nft="startCounting" @makerfound="makerFound" />
		<event-stamp v-if="stampInfoList" ref="stampEl" :stamp-info="stampInfoList" :template-type="templateType" />
		<event-mission-modal v-if="stampInfoList" ref="missionModalEl" :mission-list="stampInfoList"
			@close:modal="startProcess" />
...
export default {
	...
	setup() {
		...
		const {
			...
			isEventFinish,
		} = useImageScanningInfo({ eventIsScanEl });
		...
		const {
			...
			updateStamp,
			openMissionModal
		} = uesStampInfo({ stampEl, missionModalEl });
		...
		// 마커 확인 이벤트 재시작
		const startProcess = () => { // vue
			addScanEvent();
		}

		// 이벤트 완료 팝업
		const eventComplete = async ({ itemID }) => { // vue
			// 완료 API호출
