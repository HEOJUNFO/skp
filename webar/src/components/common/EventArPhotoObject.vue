<template>
  <a-scene embedded :mindar-face="isMindARImage == false ? 'uiError:no; uiLoading:no; uiScanning:no;' : null"
    ...
    <a-entity v-if="selectFilter.includes('star') || selectTab.includes('star')" position="0 -11 -40"
      particle-system="color: #FF0,#FF0; size:2;"></a-entity>
    <a-entity v-if="selectFilter.includes('snow') || selectTab.includes('snow')" position="0 -11 -40"
      particle-system="preset: snow; size:10; particleCount: 1000;"></a-entity>
    <a-entity v-if="selectFilter.includes('rain') || selectTab.includes('rain')" position="0 -11 -40"
