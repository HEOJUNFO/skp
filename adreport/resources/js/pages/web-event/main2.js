...
(function ($, window, document, undefined) {
    ...
    */
        ...
        $.ajax({
            ...
            if (is.isEqual(xhr.status, 200)) {
                ...
                if (is.isEqual(res.resultCode, 808)) {
                   ...
                   //참여번호가 세번 틀렸을때 처리 
                   if (is.isEqual(Number(attendCodeWrongCnt), 2)) {
                        _saveAttendButtonLog(eventId, "N");
    ...
    //사진보관함 버튼 이벤트
    $(document).on("click", "[name=photoRepositoryBtn]", function() {
        const photoUrl = "/webar/index.html#/photo-box/?eventId=" + eventId;
        if (isApp) {
            let targetUrl = is.concatStr(is.getDomain(), photoUrl, "&arData=", is.getArData(eventId));
            ocbApp.startOpenBrower(targetUrl);
        } else {
    ...
    let _goSurveyGoPage = function () {
        ...
        .done(function(response, text, xhr) {
            ...
            if (is.isEqual(xhr.status, 200)) {
                ...
                //참여번호 참여횟수 초과 에러
                if (is.isEqual(resultCode, 819)) {
                    $.hideElement("#attendCodePopupSection");
                    is.showCommonPopup(1, "attendLimitAttendCode");
                    return false;
                }
                //전화번호 참여횟수 초과 에러
                if (is.isEqual(resultCode, 854)) {
